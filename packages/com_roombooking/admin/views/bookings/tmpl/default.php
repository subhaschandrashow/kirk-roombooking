...
<form action="<?php echo JRoute::_('index.php?option=com_roombooking&view=bookings'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="table table-striped">
		...
		$canChange  = $user->authorise('core.edit.state',  'com_roombooking.booking'.$item->id);
		?>
        <tr>
        <td class="center"><?php echo JHtml::_('grid.id', $i, $item->id); ?></td>
        <td><a href="index.php?option=com_roombooking&view=booking&layout=edit&id=<?php echo $item->id ; ?>"><?php echo 'RB#'.sprintf("%04d", $item->id) ; ?></a></td>
