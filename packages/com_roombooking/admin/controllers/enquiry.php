...
class RoombookingControllerEnquiry extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit', 'com_roombooking.enquiry.'.$recordId)) {
			return true;
		}
	...
	function book()
	{
		...
		// booking addons
		$db->setQuery("SELECT * FROM `#__kirk_enquiry_addons` WHERE `enquiry_id` = '".$id."'");echo $db->getQuery();
	...
	function delete($key = NULL, $urlVar = NULL) {
		...
		foreach($cid as $id) {
			$db->setQuery("DELETE FROM `#__foxcontact_enquiries` WHERE `id` = '".$id."'");
			$db->Query();
		}
		
		$this->setRedirect('index.php?option=com_roombooking&view=enquiries' , JText::_('DELETED'));
	}
